 ...
 */
export function atom<A>(input: Rx.Observable<A>, onChange: (updatedValue: A) => void): Atom<A>
...
export function atom<A>(x: any, y?: any): Atom<A> {
    if (arguments.length == 1) {
        ...
        const theAtom = property.pipe(map(x => x), distinctUntilChanged())
...
function mkAtom<A>(observable: Rx.Observable<A>, get: () => A, modify: ( (fn: (a : A) => A) => Atom<A>), set: (a: A) => Atom<A>, eager: boolean): Atom<A> {
    ...
    theAtom.freezeUnless = (freezeUnlessFn: (a: A) => boolean) => {
        ...
        let freezingProperty = theAtom.pipe(filter(freezeUnlessFn), tap(v => {previousValue = v}));
...
function lensedAtom<A, B>(root: Atom<A>, lens: L.Lens<A, B>): Atom<B> {
    const theAtom = root.pipe(map(value => lens.get(value)), distinctUntilChanged()) as any
