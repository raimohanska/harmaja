...
describe("Rx Atom", () => {
    ...
    describe("Freezing", () => {
        it("Can be frozen on unwanted values", () => {
            const a = A.atom<string | null>("hello").freezeUnless(
                (a) => a !== null
            )
        ...
        it("Can be frozen on unwanted values (when not getting in between sets)", () => {
            const atom = A.atom<string | null>("hello").freezeUnless(
                (a) => a !== null
            )
...
describe("Rx Dependent Atom", () => {
    ...
    describe("Freezing", () => {
        it("Can be frozen on unwanted values", () => {
            var b = new Rx.BehaviorSubject("1") as Rx.BehaviorSubject<
                string | null
            >
            var atom = A.atom(b, (newValue) => b.next(newValue)).freezeUnless(
                (a) => a !== null
            )
            b.subscribe() // Dependent atoms need a subscription to remain up-to-date
        ...
        it("Complex case", () => {
            ...
            const mapped = a.pipe(
                distinctUntilChanged((x) => typeof x === "string"),
                switchMap((s) => {
                    if (typeof s === "string") {
                        return (a.freezeUnless(
                            (x) => !!x
                        ) as A.Atom<string>).view("length")
                    } else {
